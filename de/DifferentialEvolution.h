...
namespace de {
    ...
    public:
        struct Constraints {
            Constraints(double lower = 0.01, double upper = 2.0, bool isConstrained = false) :
                lower(lower),
                upper(upper),
  ...
  vector<double> read_file(string filename) {
    ...
    if (file.is_open()) {
        ...
        while (getline(file, line)) {
            ...
            if (line.find("e") != string::npos) {
                // Convert the string to a double using scientific notation
                lines.push_back(strtod(line.c_str(), NULL));
            } else {
                // Convert the string to a double without scientific notation
                lines.push_back(stod(line));
            }
        }
        file.close();
    } else {
    ...
    return lines;
        ...
        void InitPopulation(){
            ...
            vector<double> lines = read_file("best_params.txt");

            for (int i = 0; i < m_numberOfParameters; i++){
                m_population[0][i] = lines[i];
            }
            for (int j = 1; j < m_populationSize; j++){
                for (int i = 0; i < m_numberOfParameters; i++){
                    ...
                    m_population[j][i] = (*distribution)(m_generator);
                }
            }
        ...
        std::pair<std::vector<double>,std::vector<double>> Optimize(int iterations, bool verbose = true){
            ...
            for (int i = 0; i < iterations; i++){
                ...
                if (verbose){
                    deout << std::fixed << std::setprecision(8);
